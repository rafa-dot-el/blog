...
This document examines the management of PlayStation 1 games and the available formats.
It provides a comparison between =iso=, =mdf=, =cue+bin= and =chd= to determine the optimal
format for storage purposes.

The focus is on converting from =iso= and =mdf=, =cue + bin= to =chd= format.
...
** ECM
ECM files represent another compression layer applied to existing disc image formats.
...
* Emulator support
For PlayStation 1 this analysis uses /Beetle PSX/ the [[https://docs.libretro.com/library/beetle_psx_hw/][documentation on retroarch]] points
