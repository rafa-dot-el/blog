...
and analysis represents the ideal scenario. Considering the requirements:
  ...
  - The binary must include tooling to support the troubleshooting process,
...
dependencies, and includes a complete set of tooling which we can use
...
For convenience, on systems with multiple network cards, it is
